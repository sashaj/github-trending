<template>
  <div class="repo-table__item">
    <a :href="repo_data.html_url" class="repo-table__title-wrapper">
      ...
      <a :href="repo_data.html_url" class="repo-table__owner">{{repo_data.owner.login}}</a>
    ...
    <div v-if="contributorsLoaded">
      <div
        ...
        :key="index"
      >
        <a class="v-catalogue-item__image" v-for="element in item" :key="element.id" :href="element.html_url">
...
export default {
  ...
  computed: {
    ...
    getIndex() {
      let index = 0;
      for (let i = 0; i < this.contributors_data.length; i++) {
        index++;
      }
      return index;
    }
